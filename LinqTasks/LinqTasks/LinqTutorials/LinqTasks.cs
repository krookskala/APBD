...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, emp => emp.Deptno, dept => dept.Deptno,(emp, dept) => new { emp.Ename, emp.Job, DeptName = dept.Dname });
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(e => new {e.Ename, e.Job, e.HireDate}).Union(new[] { new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null } });
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.GroupBy(emp => emp.Deptno).Where(group => group.Count() > 1 && group.Key != null).Select(group => new { Name = Depts.First(dept => dept.Deptno == group.Key).Dname, NumOfEmployees = group.Count() });
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.MyGetEmpsWithSubordinates();
            return result;
        }
        ...
        {
            int result = arr.GroupBy(x => x).Where(g => g.Count() % 2 != 0).Select(g => g.Key).FirstOrDefault();
            //result=
            return result;
        }
        ...
        {
            var empCounts = Emps.GroupBy(e => e.Deptno).Select(group => new { Deptno = group.Key, Count = group.Count() }).ToList(); 
            return Depts.Where(d => !empCounts.Any(e => e.Deptno == d.Deptno) || empCounts.Any(e => e.Deptno == d.Deptno && e.Count == 5)).OrderBy(d => d.Dname);
        }
        ...
        {
            IEnumerable<object> result = Emps.SelectMany(emp => Depts, (emp, dept) => new { emp.Ename, emp.Job, dept.Dname });
            //result =
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        public static IEnumerable<Emp> MyGetEmpsWithSubordinates(this IEnumerable<Emp> emps)
        {
            var result = emps.Where(e1 => emps.Any(e2 => e2.Mgr?.Empno == e1.Empno)).OrderBy(e => e.Ename).ThenByDescending(e => e.Salary);
            return result;
        }
    }
}